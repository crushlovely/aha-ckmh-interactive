        ...
        <div class="ck-body__wrapper">
          ...
          <div class="ck-layout" :class="{ 'ck-layout--open': activeRegion }">
            ...
            <div class="ck-diagram-wrap">
              ...
              >
                <defs>
                  <!-- Individual circle clip paths (magnetic-aware) -->
                  <clipPath id="ckmh-clipH">
                    <circle class="magneticTransform" cx="175" cy="145" r="130" :transform="magneticTransform('heart')" />
                  </clipPath>
                  <clipPath id="ckmh-clipK">
                    <circle class="magneticTransform" cx="325" cy="145" r="130" :transform="magneticTransform('kidney')" />
                  </clipPath>
                  <clipPath id="ckmh-clipM">
                    <circle class="magneticTransform" cx="250" cy="270" r="130" :transform="magneticTransform('metabolic')" />
                  </clipPath>
                  ...
                  <linearGradient id="ck-shimmer-grad" x1="0" y1="0" x2="1" y2="0">
                ...
                <!-- HALO GLOWS (decorative, behind base circles) -->
                <circle
                  cx="175"
                  cy="145"
                  r="135"
                  :fill="regions.heart.color"
                  :opacity="haloOpacity('heart')"
                  class="ck-halo"
                />
                <circle
                  cx="325"
                ...
                <circle
                  ...
                  :transform="magneticTransform('heart')"
                ...
                <circle
                  ...
                  :transform="magneticTransform('kidney')"
                  class="ck-circle-base"
                  data-region="kidney"
                />
                <circle
                  ...
                  :transform="magneticTransform('metabolic')"
                  class="ck-circle-base"
                  data-region="metabolic"
                />
                ...
                <g :opacity="labelOpacity('heart')" pointer-events="none">
                  ...
                  <svg x="110" y="90" width="34" height="34" viewBox="0 0 500 500">
                    <path
                  ...
                  <text x="129" y="140" class="ck-label" text-anchor="middle">Heart</text>
                </g>

                <!-- Kidney label -->
                <g :opacity="labelOpacity('kidney')" pointer-events="none">
                  <!-- Kidneys icon (34×34) centered at 388, 101 -->
                  <svg x="351" y="90" width="34" height="34" viewBox="0 0 500 500">
                    <path
                  ...
                  <text x="368" y="140" class="ck-label" text-anchor="middle">Kidneys</text>
                </g>
                ...
                <!-- SHIMMER SWEEP (clipped to focused region) -->
                <rect
                  ...
                  width="200"
                  height="515"
                  fill="url(#ck-shimmer-grad)"
                  :clip-path="shimmerClipPath"
                  class="ck-shimmer"
                  pointer-events="none"
                />

                <!-- CLICK RIPPLE (one per organ involved in the active region) -->
                <circle
                  v-for="(organ, i) in pulseLabelOrgans"
                  :key="`pulse-${organ}-${pulseKey}`"
                  :cx="labelCenter(organ).x"
                  :cy="labelCenter(organ).y"
                  r="20"
                  fill="none"
                  :stroke="regions[organ].color"
                  stroke-width="3"
                  class="ck-pulse"
              ...
              >
                <div class="ck-panel__inner">
                  <transition name="ck-content" mode="out-in">
                    <div class="ck-panel__content" :key="activeRegion">
                      <div
...
export default {
  ...
  computed: {
    ...
    shimmerClipPath() {
      const map = {
        heart: 'ckmh-clipH',
        kidney: 'ckmh-clipK',
        metabolic: 'ckmh-clipM',
        heart_kidney: 'ckmh-clipHK',
        heart_metabolic: 'ckmh-clipHM',
        kidney_metabolic: 'ckmh-clipKM',
        center: 'ckmh-clipHKM',
      }
    ...
    diagramTiltStyle() {
      ...
      return { '--tilt-x': `${tiltX}deg`, '--tilt-y': `${tiltY}deg` }
    },
  },
  ...
  methods: {
    ...
    magneticTransform(key) {
      ...
      const radius = 180
      const transformAmount = 1
      if (dist > radius) return 'translate(0 0)'
      const pull = (1 - dist / radius) * transformAmount
      const norm = Math.max(dist, 0.01)
      const ox = ((dx / norm) * pull).toFixed(2)
      const oy = ((dy / norm) * pull).toFixed(2)
...
.ck-diagram {
  ...
  animation:
    ck-svg-entry 0.8s cubic-bezier(0.34, 1.56, 0.64, 1) both,
    ck-svg-breathe 5s ease-in-out 0.8s infinite;
}

/* Entry: pop-in scale + fade (uses individual `scale` so `transform` stays free for tilt) */
...
/* Ambient: subtle breathing pulse */
...
/* Halo glow behind base circles, lights up on focus */
.ck-halo {
  pointer-events: none;
  filter: blur(18px);
...
.ck-halo:nth-of-type(2) {
  animation-delay: 0.4s;
  animation-duration: 3.6s;
}
.ck-halo:nth-of-type(3) {
...
/* Click ripple — re-mounts on each selection (key change) */
.ck-pulse {
  transform-box: fill-box;
  transform-origin: 50% 50%;
  filter: blur(1px);
  animation: ck-pulse-grow 1s ease-out forwards;
}
...
/* Shimmer sweep — diagonal light bar across focused region */
.ck-shimmer {
...
@keyframes ck-shimmer-sweep {
  ...
  100% {
    transform: translateX(740px) skewX(-18deg);
    opacity: 0;
  }
}
...
/* ── Content transition (between region selections within an open panel) ── */
.ck-content-enter-active {
  transition:
    transform 0.32s cubic-bezier(0.2, 0.8, 0.2, 1),
    opacity 0.32s ease;
}
.ck-content-leave-active {
  transition:
    transform 0.18s ease-in,
    opacity 0.18s ease-in;
}
.ck-content-enter-from {
  opacity: 0;
